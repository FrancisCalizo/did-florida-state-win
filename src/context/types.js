// Games Context Types
export const GET_GAMES = 'GET_GAMES';
export const GET_TEAM_INFO = 'GET_TEAMINFO';
export const SET_SEASON_WINS = 'SET_SEASON_WINS';
export const SET_SEASON_LOSSES = 'SET_SEASON_LOSSES';
export const SET_HOME_WINS = 'SET_HOME_WINS';
export const SET_HOME_LOSSES = 'SET_HOME_LOSSES';
export const SET_AWAY_WINS = 'SET_AWAY_WINS';
export const SET_AWAY_LOSSES = 'SET_AWAY_LOSSES';
export const SET_CONFERENCE_WINS = 'SET_CONFERENCE_WINS';
export const SET_CONFERENCE_LOSSES = 'SET_CONFERENCE_LOSSES';
export const SET_YEAR = 'SET_YEAR';

// Game Details Types
export const GET_GAME_INFO = 'GET_GAME_INFO';
export const CLEAR_GAME_INFO = 'CLEAR_GAME_INFO';
export const NO_INFO_AVAILABLE = 'NO_INFO_AVAILABLE';

// FSU Team Types
export const GET_FSU_TEAM_INFO = 'GET_FSU_TEAM_INFO';

// Roster Types
export const GET_ROSTER_INFO = 'GET_ROSTER_INFO';
export const NO_ROSTER_INFO_AVAILABLE = 'NO_ROSTER_INFO_AVAILABLE';

// DidWeWinTypes
export const FETCH_CURRENT_SCHEDULE = 'FETCH_CURRENT_SCHEDULE';
export const FETCH_CURRENT_GAME = 'FETCH_CURRENT_GAME';
export const FETCH_NEXT_GAME = 'FETCH_NEXT_GAME';
export const FETCH_LAST_GAME = 'FETCH_LAST_GAME';
export const FETCH_TIME_UNTIL_NEXT_GAME = 'FETCH_TIME_UNTIL_NEXT_GAME';
export const FETCH_LAST_GAME_OPPONENT = 'FETCH_LAST_GAME_OPPONENT';
export const FETCH_NEXT_GAME_OPPONENT = 'FETCH_NEXT_GAME_OPPONENT';
export const FETCH_CURRENT_GAME_OPPONENT = 'FETCH_CURRENT_GAME_OPPONENT';
export const FETCH_FSU_INFO = 'FETCH_FSU_INFO';
